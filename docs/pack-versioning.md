...
**Installer:** Run with `cursor-hub install` (after `pip install -e .`) or `python tools/install.py` from the hub repo. Merges by copy; prints installed pack versions from `pack.yml` when present. Same filename = last pack wins unless `--overwrite`. Copies scripts into the target's `.cursor/tools/`.
