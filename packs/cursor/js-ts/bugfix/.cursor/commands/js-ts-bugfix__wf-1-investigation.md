...
6. **Record in design log (mandatory):** Create or update the bugfix log for this workstream. If no log exists, run `python .cursor/tools/new_design_log.py --slug bugfix-<short-name> --kind bugfix` (or fallback: create the file in `./.cursor/design-log/`). Fill/update the fields under: **Impact**, **Detection / repro**, **Root cause** (include hypotheses if still uncertain). Create logs only in `./.cursor/design-log/`. Do not ask — do it. **Workflow stamp:** After recording, append exactly one standalone line `[cursor-hub workflow] step=investigation` (required so the validator accepts slim logs; legacy logs with old section headings only still pass without it). 
7. **Verify:** Run `python .cursor/tools/validate_workflow_design_log.py --step investigation` from the project root. If it fails, add the missing impact/detection/root-cause content and/or the workflow stamp line and re-run the validator. Do not consider the command complete until the validator passes.
