...
5. **Record in design log (mandatory):** Update/append under **Verification**: what was covered, how tests were run, any failures fixed. If no log exists, run `python .cursor/tools/new_design_log.py --slug <short-name>` (or fallback: create the file in `./.cursor/design-log/`). Create logs only in `./.cursor/design-log/`. Do not ask — do it. **Workflow stamp:** After recording, append exactly one standalone line `[cursor-hub workflow] step=add-tests` (required so the validator accepts slim logs; legacy logs with old section headings only still pass without it).
