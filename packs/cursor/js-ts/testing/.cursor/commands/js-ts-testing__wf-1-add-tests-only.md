...
4. **Verification:** Run `npm test`. Ensure no existing tests break. Run lint/typecheck on the test code if the project uses them.
5. **Record in design log (mandatory):** Update/append under **Verification**: what was covered, how tests were run, any failures fixed. If no log exists, run `python .cursor/tools/new_design_log.py --slug <short-name>` (or fallback: create the file in `./.cursor/design-log/`). Create logs only in `./.cursor/design-log/`. Do not ask — do it.
6. **Verify:** Run `python .cursor/tools/validate_workflow_design_log.py --step add-tests` from the project root. If it fails, add the missing verification content and re-run the validator. Do not consider the command complete until the validator passes.
