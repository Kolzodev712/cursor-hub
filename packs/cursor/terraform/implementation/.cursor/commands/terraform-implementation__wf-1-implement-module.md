...
4. **Per phase:**
   ...
   - Run verification: `terraform fmt -recursive -check` (then `terraform fmt -recursive` if needed), `terraform validate`. Optionally `terraform plan` (no apply). Fix any failures.
...
6. **Record in design log (mandatory):** Append the design log under **Implementation results**: what was done, deviations from the plan, verification outcome. If no design log exists for this workstream, run `python .cursor/tools/new_design_log.py --slug <short-name>` (or fallback: create the file in `./.cursor/design-log/`). Create logs only in `./.cursor/design-log/`. Do not consider the command complete until the log is updated. **Workflow stamp:** After recording, append exactly one standalone line `[cursor-hub workflow] step=implement` (required so the validator accepts slim logs; legacy logs with old section headings only still pass without it).
7. **Verify:** Run `python .cursor/tools/validate_workflow_design_log.py --step implement` from the project root. Use the repo root or the Terraform module root as appropriate. If it fails, add the missing Implementation results content and/or the workflow stamp line and re-run the validator. Do not consider the command complete until the validator passes.
