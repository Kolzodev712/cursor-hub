...
Create a new design-log entry with a **slim template**. Design log location and structure are defined in the **design-log** rule (`_shared`); this command follows that specification.
...
1. **Create the file:** From the project root, run:
   - Decision / feature log (default): `python .cursor/tools/new_design_log.py --slug <short-name>`
   - Bugfix log: `python .cursor/tools/new_design_log.py --slug bugfix-<short-name> --kind bugfix`
...
3. **Fill the template** with the user's content. Keep it short and durable (no transcripts).
   - Decision / feature log: Problem / context, Decision, Alternatives, Consequences, Verification (Implementation results appended later).
   - Bugfix log: Impact, Detection / repro, Root cause, Fix, Caveats / follow-ups, Verification.

4. **Be specific:** Include file paths, types, and verification criteria as appropriate.

If the user provided a title or outline, use it to populate the sections. If not, add 1–2 sentence stubs so the user can edit.
