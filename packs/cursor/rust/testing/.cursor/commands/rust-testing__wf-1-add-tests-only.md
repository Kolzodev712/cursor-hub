...
5. **Record in design log (mandatory):** Update/append under **Verification**: what was covered (e.g. edge cases, fixtures), how tests were run, any failures fixed. If no log exists, run `python .cursor/tools/new_design_log.py --slug <short-name>` (or fallback: list `./.cursor/design-log/`, take max NNN + 1, create the file). Create logs only in `./.cursor/design-log/`. Do not ask — do it.
